...
2026-09-12 阶段性交付：本分支已包含仓库/货位资料管理、库存台账查询、FEFO 预览、效期预警、三账核对、盘点审批调整、报损审批复核执行、页面 API、规则和 Mapper 绑定测试。库存目录格式、ESLint 与 Vite 构建通过；全仓 TypeScript 仍有其他模块错误。本机 Docker Compose 配置可解析，但 daemon 尚未启动，真实 MySQL 和实际登录验收尚未完成；收货/销售/退货写入闭环、上架移位、页面截图仍未完成，完整缺口见 11。
