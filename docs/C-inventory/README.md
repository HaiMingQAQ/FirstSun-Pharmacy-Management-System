...
2026-09-13 阶段性交付：本分支已包含仓库/货位资料管理（含空资料安全删除）、库存台账查询、FEFO 预览、效期预警、三账核对、盘点审批调整、报损审批复核执行、三账导出、页面 API、规则和 Mapper 绑定测试。库存目录 Prettier、ESLint 与 Vite 构建通过；Docker Java17 后端镜像构建、启动及 MySQL/Redis 连接通过，OpenAPI 已注册 C 路由。本地 MySQL 当前库存作业表为空，尚未进行带真实员工登录和业务夹具的事务演示；收货/销售/退货写入闭环、上架移位、页面截图仍未完成，完整缺口见 11。
...
开发接续请先读 [当前工作区、实现与缺口清单](./11-新目录恢复与缺口清单.md)、[独立 MySQL 验收入口](./12-独立MySQL验收入口.md) 和 [同仓上架移位](./13-同仓上架移位.md)。以下保留原启动阶段说明，不代表当前开发进度。
